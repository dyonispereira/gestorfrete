...
class DeleteClientContactCommand(Command):
    actor: AuthenticatedActor
    client_id: uuid.UUID
    contact_id: uuid.UUID


class DeleteClientContactHandler(CommandHandler[DeleteClientContactCommand, None]):
    async def handle(self, command: DeleteClientContactCommand) -> None:
        async with SQLAlchemyUnitOfWork() as uow:
            ...
            contact.soft_delete(now=datetime.now(timezone.utc))
