...
class ValidateCteCommand(Command):
    actor: AuthenticatedActor
    cte_id: uuid.UUID


class ValidateCteHandler(CommandHandler[ValidateCteCommand, CteDTO]):
    def __init__(self, audit_logger: AuditLogger | None = None) -> None:
        self._audit = audit_logger or AuditLogger()

    async def handle(self, command: ValidateCteCommand) -> CteDTO:
