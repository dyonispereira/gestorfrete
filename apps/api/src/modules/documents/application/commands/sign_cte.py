...
class SignCteCommand(Command):
    actor: AuthenticatedActor
    cte_id: uuid.UUID


class SignCteHandler(CommandHandler[SignCteCommand, CteDTO]):
    """`VALIDADO→ASSINADO`. D279 — usa o certificado já configurado em `FiscalConfiguration`, nunca
    aceita certificado no corpo da requisição."""

    def __init__(self, audit_logger: AuditLogger | None = None) -> None:
        self._audit = audit_logger or AuditLogger()

    async def handle(self, command: SignCteCommand) -> CteDTO:
        async with SQLAlchemyUnitOfWork() as uow:
            ...
            config = await config_repo.get_for_tenant()
            if config is not None and config.certificado_validade < date.today():
                raise ConflictError("FISCAL_CERTIFICATE_EXPIRED", "Certificado digital do tenant está vencido.")
