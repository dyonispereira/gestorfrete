...
class InterromperTripCommand(Command):
    actor: AuthenticatedActor
    trip_id: uuid.UUID
    notes: str


class InterromperTripHandler(CommandHandler[InterromperTripCommand, TripDTO]):
    """`commands/interromper` — `notes` obrigatória (D007, `viagem_status_history.observacao`)."""

    def __init__(self, audit_logger: AuditLogger | None = None) -> None:
        self._audit = audit_logger or AuditLogger()

    async def handle(self, command: InterromperTripCommand) -> TripDTO:
        if not command.notes or not command.notes.strip():
            raise ValidationError("FREIGHT_TRIP_NOTES_REQUIRED", "notes é obrigatória para interromper a viagem.")
