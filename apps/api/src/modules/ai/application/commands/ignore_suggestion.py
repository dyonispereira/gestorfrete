...
class IgnoreSuggestionCommand(Command):
    actor: AuthenticatedActor
    suggestion_id: uuid.UUID


class IgnoreSuggestionHandler(CommandHandler[IgnoreSuggestionCommand, AISuggestionDTO]):
    def __init__(self, audit_logger: AuditLogger | None = None) -> None:
        self._audit = audit_logger or AuditLogger()

    async def handle(self, command: IgnoreSuggestionCommand) -> AISuggestionDTO:
        async with SQLAlchemyUnitOfWork() as uow:
            ...
            suggestion.ignore(usuario_id=command.actor.user_id, now=datetime.now(timezone.utc))
            ...
            await self._audit.record(
                ...
                ator_nome_snapshot=str(command.actor.user_id), dados_depois={"status": "IGNORADA"},
            )
            await uow.commit()
