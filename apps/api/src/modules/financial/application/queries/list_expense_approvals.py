...
class ListExpenseApprovalsQuery(Query):
    actor: AuthenticatedActor
    accounts_payable_id: uuid.UUID


class ListExpenseApprovalsHandler(QueryHandler[ListExpenseApprovalsQuery, list[ExpenseApprovalDTO]]):
    def __init__(self, session_factory: async_sessionmaker[AsyncSession]) -> None:
        self._session_factory = session_factory

    async def handle(self, query: ListExpenseApprovalsQuery) -> list[ExpenseApprovalDTO]:
        async with self._session_factory() as session:
            ...
            approval_repo = SqlAlchemyExpenseApprovalRepository(session)
            approvals = await approval_repo.list_for_payable(query.accounts_payable_id)
        return [ExpenseApprovalDTO.from_entity(a) for a in approvals]
