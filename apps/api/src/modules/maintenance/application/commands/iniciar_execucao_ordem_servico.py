...
class IniciarExecucaoOrdemServicoCommand(Command):
    actor: AuthenticatedActor
    ordem_servico_id: uuid.UUID


class IniciarExecucaoOrdemServicoHandler(CommandHandler[IniciarExecucaoOrdemServicoCommand, OrdemServicoDTO]):
    """`EM_DIAGNOSTICO→EM_EXECUCAO` — só quando `necessita_aprovacao=False` (validação de
    aplicação; o caminho com aprovação passa por `AGUARDANDO_APROVACAO`)."""

    def __init__(self, audit_logger: AuditLogger | None = None) -> None:
        self._audit = audit_logger or AuditLogger()

    async def handle(self, command: IniciarExecucaoOrdemServicoCommand) -> OrdemServicoDTO:
        async with SQLAlchemyUnitOfWork() as uow:
            ...
            ordem_servico.iniciar_execucao(now=now, atualizado_por=command.actor.user_id)
