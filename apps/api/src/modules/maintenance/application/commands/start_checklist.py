...
class StartChecklistCommand(Command):
    actor: AuthenticatedActor
    checklist_id: uuid.UUID


class StartChecklistHandler(CommandHandler[StartChecklistCommand, ChecklistDTO]):
    """`PENDENTE→EM_PREENCHIMENTO`."""

    def __init__(self, audit_logger: AuditLogger | None = None) -> None:
        self._audit = audit_logger or AuditLogger()

    async def handle(self, command: StartChecklistCommand) -> ChecklistDTO:
