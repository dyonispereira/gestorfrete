...
class ReprovarCustoOrdemServicoCommand(Command):
    actor: AuthenticatedActor
    ordem_servico_id: uuid.UUID
    justificativa: str


class ReprovarCustoOrdemServicoHandler(CommandHandler[ReprovarCustoOrdemServicoCommand, OrdemServicoDTO]):
    """`AGUARDANDO_APROVACAO→EM_DIAGNOSTICO` — volta para diagnóstico, nunca cancela sozinha."""

    def __init__(self, audit_logger: AuditLogger | None = None) -> None:
        self._audit = audit_logger or AuditLogger()

    async def handle(self, command: ReprovarCustoOrdemServicoCommand) -> OrdemServicoDTO:
        if not command.justificativa or not command.justificativa.strip():
            raise ValidationError(
                "MAINTENANCE_WORK_ORDER_JUSTIFICATIVA_REQUIRED", "justificativa é obrigatória para reprovar custo."
            )
        ...
        async with SQLAlchemyUnitOfWork() as uow:
            ...
            ordem_servico.reprovar_custo(now=now, atualizado_por=command.actor.user_id)
            await os_repo.add(ordem_servico)

            await aprovacao_repo.add(
                AprovacaoCusto.create(
                    ordem_servico_id=ordem_servico.id, decisao=AprovacaoCustoDecisao.REJEITADO,
