...
class SubmeterAprovacaoOrdemServicoCommand(Command):
    actor: AuthenticatedActor
    ordem_servico_id: uuid.UUID


class SubmeterAprovacaoOrdemServicoHandler(CommandHandler[SubmeterAprovacaoOrdemServicoCommand, OrdemServicoDTO]):
    """`EM_DIAGNOSTICO→AGUARDANDO_APROVACAO`."""

    def __init__(self, audit_logger: AuditLogger | None = None) -> None:
        self._audit = audit_logger or AuditLogger()

    async def handle(self, command: SubmeterAprovacaoOrdemServicoCommand) -> OrdemServicoDTO:
        async with SQLAlchemyUnitOfWork() as uow:
            ...
            ordem_servico.submeter_aprovacao(now=now, atualizado_por=command.actor.user_id)
