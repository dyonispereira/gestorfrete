...
class UnblockDriverCommand(Command):
    actor: AuthenticatedActor
    driver_id: uuid.UUID


class UnblockDriverHandler(CommandHandler[UnblockDriverCommand, DriverDTO]):
    def __init__(self, audit_logger: AuditLogger | None = None) -> None:
        self._audit = audit_logger or AuditLogger()

    async def handle(self, command: UnblockDriverCommand) -> DriverDTO:
        async with SQLAlchemyUnitOfWork() as uow:
            ...
            driver.unblock(unblocked_by=command.actor.user_id, now=datetime.now(timezone.utc))
