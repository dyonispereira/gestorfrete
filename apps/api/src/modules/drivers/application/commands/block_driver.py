...
class BlockDriverCommand(Command):
    actor: AuthenticatedActor
    driver_id: uuid.UUID


class BlockDriverHandler(CommandHandler[BlockDriverCommand, DriverDTO]):
    """Bloqueio é auditoria crítica (`RBAC_MATRIX.md`: criticidade Alta, aprovação Gerente
    Operacional) — sempre grava `logs_auditoria`, mesmo sendo um único campo alterado
    (`DRIVER_IMPLEMENTATION.md`)."""

    def __init__(self, audit_logger: AuditLogger | None = None) -> None:
        self._audit = audit_logger or AuditLogger()

    async def handle(self, command: BlockDriverCommand) -> DriverDTO:
        async with SQLAlchemyUnitOfWork() as uow:
            ...
            driver.block(blocked_by=command.actor.user_id, now=datetime.now(timezone.utc))
            ...
            await self._audit.record(
                ...
                dados_depois={"fitness_status": driver.fitness_status.value},
            )
