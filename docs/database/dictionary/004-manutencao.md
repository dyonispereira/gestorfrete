...
| ORDEM_SERVICO.HODOMETRO_ABERTURA_KM | Hodômetro na abertura | Decimal | Abertura | Não | Informado | Não | Interno | Reconciliado — quando informado, também gera uma Leitura de Hodômetro real em `fleet` (`origem = ORDEM_SERVICO`, D033/D034: este campo é denormalização de exibição, a Time Series é dona do dado histórico) |
| ORDEM_SERVICO.HODOMETRO_CONCLUSAO_KM | Hodômetro na conclusão | Decimal | Execução | Não | Informado | Não | Interno | Reconciliado — mesmo princípio de `HODOMETRO_ABERTURA_KM`; par que alimenta o futuro Plano de Manutenção Preventiva por KM |
| ORDEM_SERVICO.CENTRO_CUSTO_ID | Centro de Custo | Referência | Abertura/Diagnóstico | Não | Informado | Não | Interno | Reconciliado (Lote Financeiro, Parte 1) — FK opcional para `centros_custo` (`001-cadastros.md`). Quando preenchido junto com `FORNECEDOR_EXECUTOR_ID`, habilita a Conta a Pagar automática no fechamento (`006-financeiro.md`); quando ausente, o fechamento segue normalmente, só sem gerar a Conta a Pagar sozinha |
