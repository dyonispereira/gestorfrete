...
| LEITURA_HODOMETRO.VALOR_KM | Valor do hodômetro | Decimal | Sim | Capturado (abastecimento/checklist/manual) | **É ela própria o histórico** — nunca editada, apenas inserida (D037) | Interno | Invariante: nunca menor que a última leitura do mesmo veículo |
| LEITURA_HODOMETRO.ORIGEM | Origem da leitura | Enum | Sim | Capturado (sistema) | Não | Interno | Valores: Abastecimento/Checklist/Manual/Telemetria (futuro) |
| LEITURA_HODOMETRO.VIAGEM_ID | Viagem associada | Referência | Não | Capturado (sistema) | Não | Interno | Quando a leitura ocorre no contexto de uma Viagem específica — origem funcional de `KM_INICIAL`/`KM_FINAL` (D077, atributos críticos citados como exemplo): a primeira Leitura de uma Viagem é o `KM_INICIAL`, a última é o `KM_FINAL`; não são campos próprios da Viagem, são a leitura de fronteira desta entidade histórica |
